class Population {
  ...
  generate() {
    ...
    for (let index in this.population) {
      population[index] = this.population.length - index > Math.ceil(this.population.length * this.toBeRandomized) ? new this.instantiator().setDeathHandler(this.onDeath.bind(this)) : this.poolSelection();
