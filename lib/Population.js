class Population {
  constructor(phenotype, size, mutationRate, crossoverRate, context) {
    this.best = null;
    ...
    for (let i = 0; i < size; i++) {
      this.population[i] = new phenotype().setDeathHandler(this.onDeath.bind(this));
  ...
  generate() {
    ...
    for (let index in this.population) {
      ...
      population[index] = this.phenotype.crossover(mother, father, this.crossoverRate).mutate(this.mutationRate);
