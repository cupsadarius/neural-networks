...
class NeuralNetwork {
  ...
  constructor(input, hidden_nodes_on_layer, output) {

    this.nodes = [input];
    for (let i = 0; i < hidden_nodes_on_layer.length; i++) {
      this.nodes[i + 1] = hidden_nodes_on_layer[i];
    }
    this.nodes[this.nodes.length] = output;
  ...
  static fromNeuralNetwork(network) {
    let output = new NeuralNetwork(network.input_nodes, network.hidden_nodes, network.output_nodes);
    output.input_nodes = network.input_nodes;
    output.hidden_nodes = network.hidden_nodes;
    output.output_nodes = network.output_nodes;

    output.weights_ih = network.weights_ih.copy();
    output.weights_ho = network.weights_ho.copy();

    output.bias_h = network.bias_h.copy();
    output.bias_o = network.bias_o.copy();
