class NeuralAgent extends Agent {
  ...
  mutate(mutationRate, layerMutationRate, nodeMutationRate) {
    ...
    if (Math.random() < nodeMutationRate && dna.nodes.length > 2) {
      const layerToChange = randomInt(1, dna.nodes.length - 1);
      const action = Math.round(randomFloat(-1, 1));
      nodes = dna.nodes.slice();
      nodes[layerToChange] = nodes[layerToChange] + action > 0 ? nodes[layerToChange] + action : nodes[layerToChange];
    }
