  ...
  mutate(mutationRate, structuralOptions) {
    ...
    if (structuralOptions) {
      ...
      if (Math.random() < structuralOptions.layerMutationRate) {
        ...
        const action = randomFloat(0, 1) < structuralOptions.decreaseLayerRate ? -1 : 1;
    ...
    dna.mutate((e) => Math.random() < mutationRate ? e + (Array(10).fill(Math.random()).reduce((acc, e) => acc + e, 0) / 10) * 0.5 : e);
    this.dna = dna;
